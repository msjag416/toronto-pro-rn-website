          ...
          <div className="flex justify-between items-center">
            ...
            <div className="hidden md:flex items-center space-x-8">
              {['Services', 'Verification', 'About', 'FAQ'].map((item) => (
                <button key={item} onClick={() => scrollToSection(item.toLowerCase())} className="text-sm font-medium text-slate-600 hover:text-blue-600 transition-colors">{item}</button>
              ))}
              <button onClick={() => scrollToSection('contact')} className="bg-blue-600 hover:bg-blue-700 text-white px-5 py-2.5 rounded-full text-sm font-semibold transition-all shadow-lg shadow-blue-600/20">Hire a Nurse</button>
            </div>
            <div className="md:hidden">
              <button onClick={() => setIsMenuOpen(!isMenuOpen)} className="text-slate-700 p-2">{isMenuOpen ? <X size={24} /> : <Menu size={24} />}</button>
            </div>
          </div>
        </div>
        {isMenuOpen && (
          <div className="md:hidden absolute top-full left-0 w-full bg-white border-b border-slate-100 shadow-xl">
            <div className="px-4 py-6 space-y-4">
              {['Services', 'Verification', 'About', 'FAQ'].map((item) => (
                <button key={item} onClick={() => scrollToSection(item.toLowerCase())} className="block w-full text-left text-lg font-medium text-slate-600 py-2 border-b border-slate-50">{item}</button>
              ))}
              <button onClick={() => scrollToSection('contact')} className="w-full bg-blue-600 text-white py-3 rounded-lg font-semibold mt-4">Hire a Nurse</button>
            </div>
          </div>
        )}
      </nav>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div>
              ...
              <h1 className="text-4xl lg:text-6xl font-extrabold text-slate-900 leading-[1.1] mb-6">Hospital-Grade <span className="text-blue-600">ICU Care</span> at Home.</h1>
              <p className="text-lg text-slate-600 mb-8 leading-relaxed max-w-lg">Don't settle for basic care. Hire EXPERIENCED ICU nurses in Toronto for ventilated loved ones, high acuity needs, and complex medical answers.</p>
              <div className="flex flex-col sm:flex-row gap-4">
                <button onClick={() => scrollToSection('contact')} className="inline-flex justify-center items-center px-8 py-4 bg-blue-600 text-white font-bold rounded-lg hover:bg-blue-700 transition-all shadow-lg shadow-blue-600/30">Find a Nurse Now</button>
                <button onClick={() => scrollToSection('verification')} className="inline-flex justify-center items-center px-8 py-4 bg-white text-slate-700 border border-slate-200 font-semibold rounded-lg hover:bg-slate-50 transition-all">Verify Credentials</button>
              </div>
              <div className="mt-10 flex items-center space-x-4 text-sm text-slate-500">
                <div className="flex -space-x-2">
                  {[1,2,3,4].map(i => (<div key={i} className={`w-8 h-8 rounded-full border-2 border-white bg-slate-200 flex items-center justify-center text-[10px] font-bold text-slate-400`}>RN</div>))}
                </div>
            ...
            <div className="relative">
              <div className="relative rounded-2xl overflow-hidden shadow-2xl shadow-blue-900/10 bg-slate-100 aspect-[4/3] group">
                {/* HERO IMAGE: Drive link processed for preview. On Netlify/GitHub, use local file './hero.jpg' */}
                <img src="https://lh3.googleusercontent.com/d/1-sDTqxsvAncbOK4-hALcS4vZvRhXKJyt" alt="Specialized ICU nurse in blue scrubs adjusting ventilator mask on elderly patient in a home setting" className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" />
      ...
      <section className="bg-slate-900 py-12 text-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 text-center">
            <div><div className="text-3xl font-bold text-blue-400 mb-1">100%</div><div className="text-slate-400 text-sm">CNO Registered</div></div>
            <div><div className="text-3xl font-bold text-blue-400 mb-1">24/7</div><div className="text-slate-400 text-sm">High Acuity Support</div></div>
            <div><div className="text-3xl font-bold text-blue-400 mb-1">ICU</div><div className="text-slate-400 text-sm">Specialized Experience</div></div>
            <div><div className="text-3xl font-bold text-blue-400 mb-1">GTA</div><div className="text-slate-400 text-sm">Toronto & Area</div></div>
          </div>
        </div>
      </section>
      ...
      <section id="services" className="py-24 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center max-w-3xl mx-auto mb-16">
            ...
            <p className="text-lg text-slate-600">General care isn't enough for ventilated or high-acuity patients. You need an expert who understands the machines, the numbers, and the subtle signs of change.</p>
          </div>
          <div className="grid md:grid-cols-3 gap-8">
            <div className="bg-slate-50 rounded-2xl p-8 transition-all hover:-translate-y-1 hover:shadow-xl border border-slate-100">
              <div className="w-14 h-14 bg-blue-100 rounded-xl flex items-center justify-center mb-6"><Activity className="w-7 h-7 text-blue-600" /></div>
              <h4 className="text-xl font-bold text-slate-900 mb-3">Ventilator & Trach Care</h4>
              <p className="text-slate-600 leading-relaxed mb-4">Specialized support for loved ones dependent on mechanical ventilation. We manage airway clearance, cuff pressures, and emergency troubleshooting.</p>
            ...
            <div className="bg-slate-50 rounded-2xl p-8 transition-all hover:-translate-y-1 hover:shadow-xl border border-slate-100">
              <div className="w-14 h-14 bg-teal-100 rounded-xl flex items-center justify-center mb-6"><Stethoscope className="w-7 h-7 text-teal-600" /></div>
              <h4 className="text-xl font-bold text-slate-900 mb-3">High Acuity ICU Nursing</h4>
              <p className="text-slate-600 leading-relaxed mb-4">Bring the ICU skill set home. We monitor hemodynamics, manage complex medication regimens (IVs, PICC lines), and provide critical assessments.</p>
              <ul className="space-y-2 text-sm text-slate-700">
            ...
            <div className="bg-slate-50 rounded-2xl p-8 transition-all hover:-translate-y-1 hover:shadow-xl border border-slate-100">
              <div className="w-14 h-14 bg-indigo-100 rounded-xl flex items-center justify-center mb-6"><Shield className="w-7 h-7 text-indigo-600" /></div>
              <h4 className="text-xl font-bold text-slate-900 mb-3">Consultation & Advocacy</h4>
              <p className="text-slate-600 leading-relaxed mb-4">Got questions? We provide **clear** answers. We act as your medical translator and advocate during hospital visits or care planning meetings.</p>
              <ul className="space-y-2 text-sm text-slate-700">
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="space-y-6">
            <div className="border border-slate-200 rounded-xl p-6 shadow-sm">
              <h4 className="flex items-center text-lg font-bold text-slate-900 mb-2"><CircleHelp className="w-5 h-5 text-blue-500 mr-2 flex-shrink-0" />What is the difference between an ICU Nurse and a regular RN?</h4>
              <p className="text-slate-600 pl-7">An ICU Nurse has specialized training and experience working in Intensive Care Units, which means they are highly skilled in managing life-support equipment like ventilators, interpreting complex patient data (hemodynamics), and rapidly responding to medical emergencies. A regular RN might have general training, but ICU care is a specialized skill set crucial for high-acuity patients.</p>
            </div>
            <div className="border border-slate-200 rounded-xl p-6 shadow-sm">
              <h4 className="flex items-center text-lg font-bold text-slate-900 mb-2"><CircleHelp className="w-5 h-5 text-blue-500 mr-2 flex-shrink-0" />Are your services covered by OHIP?</h4>
              <p className="text-slate-600 pl-7">No, our private nursing services are not directly covered by OHIP, as they fall outside the scope of publicly funded home care. However, many private insurance plans or employer health spending accounts may cover the costs, so we recommend you check your policy details. We operate on a private contract basis to ensure the highest standard of one-on-one care is delivered promptly.</p>
            </div>
            <div className="border border-slate-200 rounded-xl p-6 shadow-sm">
              <h4 className="flex items-center text-lg font-bold text-slate-900 mb-2"><CircleHelp className="w-5 h-5 text-blue-500 mr-2 flex-shrink-0" />How quickly can you staff a nurse?</h4>
              <p className="text-slate-600 pl-7">For urgent, high-acuity needs like ventilator management, we aim to match you with a qualified RN within 24 to 48 hours within the Greater Toronto Area. Non-urgent consultations or staffing for less critical needs can typically be arranged within three to five business days. We understand time is often critical when dealing with complex health issues, and we prioritize speed without sacrificing quality.</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section id="verification" className="py-24 bg-blue-600 relative overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-center">
          <div className="bg-white rounded-3xl p-8 md:p-12 shadow-2xl">
            <div className="inline-flex items-center justify-center p-3 bg-blue-100 rounded-full mb-6"><UserCheck className="w-8 h-8 text-blue-700" /></div>
            <h2 className="text-3xl font-bold text-slate-900 mb-4">Verify Your Nurse's License</h2>
            <p className="text-slate-600 mb-8 max-w-2xl mx-auto">Safety is paramount. Every nurse we refer is in good standing. Use the College of Nurses of Ontario (CNO) registry to confirm the status of any nursing professional.</p>
            <form onSubmit={handleVerification} className="max-w-md mx-auto">
              <div className="flex flex-col space-y-4">
                <div className="text-left">
                  ...
                  <div className="relative">
                    <input type="text" id="nurse-name" placeholder="e.g. Smith" value={nurseName} onChange={(e) => setNurseName(e.target.value)} className="w-full pl-10 pr-4 py-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-shadow" />
                    <Search className="absolute left-3 top-3.5 w-5 h-5 text-slate-400" />
                  </div>
                  <p className="text-xs text-slate-500 mt-2 ml-1">*This will open the official CNO Registry "Find a Nurse" page.</p>
                </div>
                <button type="submit" className="w-full bg-slate-900 hover:bg-slate-800 text-white font-bold py-3 px-6 rounded-lg transition-colors flex items-center justify-center">Search CNO Registry <ChevronRight className="ml-2 w-4 h-4" /></button>
              </div>
            </form>
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid lg:grid-cols-2 gap-16">
            <div>
              ...
              <p className="text-lg text-slate-600 mb-8">Tell us about your loved one's needs. We respond within 1 hour for urgent ICU requests in the Toronto area.</p>
              <div className="space-y-6">
                <div className="flex items-start">
                  <div className="bg-blue-100 p-3 rounded-lg mr-4"><MapPin className="w-6 h-6 text-blue-600" /></div>
                  <div><h4 className="font-bold text-slate-900">Service Area</h4><p className="text-slate-600">Toronto, North York, Scarborough, Etobicoke, Mississauga.</p></div>
                </div>
                <div className="flex items-start">
                  ...
                  <div>
                    ...
                    <a href="https://wa.me/14168786704?text=Hi%20I%20found%20you%20on%20your%20website" target="_blank" rel="noopener noreferrer" className="inline-block mt-2 bg-green-600 hover:bg-green-700 text-white text-sm font-bold py-2 px-4 rounded-lg transition-colors">Message us on WhatsApp</a>
              ...
              <div className="mt-12 p-6 bg-slate-50 rounded-xl border border-slate-100">
                <div className="flex items-center space-x-2 mb-2"><Star className="w-5 h-5 text-yellow-400 fill-current" /><Star className="w-5 h-5 text-yellow-400 fill-current" /><Star className="w-5 h-5 text-yellow-400 fill-current" /><Star className="w-5 h-5 text-yellow-400 fill-current" /><Star className="w-5 h-5 text-yellow-400 fill-current" /></div>
            ...
            <div className="bg-white rounded-2xl shadow-xl border border-slate-100 p-8">
              <div className="mb-6 border-b border-slate-100 pb-4">
                <div className="inline-flex items-center justify-center p-2 bg-blue-100 rounded-full mb-3"><Mail className="w-6 h-6 text-blue-600" /></div>
              ...
              <form action="https://formsubmit.co/prorninc@gmail.com" method="POST" className="space-y-6">
                ...
                <div><label className="block text-sm font-semibold text-slate-700 mb-2">Email Address</label><input type="email" name="Email" required className="w-full px-4 py-3 rounded-lg bg-slate-50 border border-slate-200 focus:border-blue-500 focus:bg-white focus:ring-2 focus:ring-blue-200 outline-none transition-all" /></div>
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {['Ventilator / Trach', 'Palliative Care', 'Post-ICU Rehab', 'Medication Mgmt'].map((need) => (
                      <label key={need} className="flex items-center space-x-2 cursor-pointer"><input type="checkbox" name="Needs[]" value={need} className="w-4 h-4 text-blue-600 rounded border-slate-300 focus:ring-blue-500" /><span className="text-slate-600 text-sm">{need}</span></label>
                    ))}
                  </div>
                </div>
                <div><label className="block text-sm font-semibold text-slate-700 mb-2">Message</label><textarea rows="4" name="Message" required className="w-full px-4 py-3 rounded-lg bg-slate-50 border border-slate-200 focus:border-blue-500 focus:bg-white focus:ring-2 focus:ring-blue-200 outline-none transition-all" placeholder="Tell us about the patient's condition and your specific needs..."></textarea></div>
                <button type="submit" className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-4 rounded-lg transition-all shadow-lg hover:shadow-xl transform hover:-translate-y-0.5">Request a Consultation</button>
                <p className="text-xs text-center text-slate-400">Your information is HIPAA/PHIPA compliant and strictly confidential © 2026.</p>
              </form>
            </div>
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid md:grid-cols-4 gap-8 mb-8">
            <div className="col-span-2">
              <div className="flex items-center space-x-2 mb-4"><HeartPulse className="w-6 h-6 text-blue-500" /><span className="text-xl font-bold text-white">Toronto<span className="text-blue-500">ProRN</span></span></div>
              <p className="text-slate-400 max-w-sm mb-4">Providing expert, licensed ICU nursing care for families in Toronto, Ontario. Specialized in ventilation, tracheostomy, and high-acuity patient support at home.</p>
            </div>
            <div>
          ...
          <div className="pt-8 border-t border-slate-800 text-center text-sm text-slate-500">Your information is HIPAA/PHIPA compliant and strictly confidential © 2026.</div>
        </div>
      </footer>
      ...
      {activeModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-sm">
          <div className="bg-white rounded-2xl w-full max-w-2xl max-h-[80vh] overflow-y-auto shadow-2xl p-8 relative animate-fade-in-up">
            <button onClick={closeModal} className="absolute top-4 right-4 p-2 bg-slate-100 rounded-full hover:bg-slate-200 transition-colors"><X className="w-6 h-6 text-slate-600" /></button>
            ...
            <div className="mt-8 pt-4 border-t border-slate-100 flex justify-end">
              <button onClick={closeModal} className="px-6 py-2 bg-blue-600 text-white font-bold rounded-lg hover:bg-blue-700 transition-colors">Close</button>
            </div>
          </div>
        </div>
      )}
